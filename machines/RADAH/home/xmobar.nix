...
in ''
  Config {
    ...
    template = " %disku% | %cpu% | %memory%  } { DNS Hours: %dns_hours% | <fc=${theme.PURPLE_COLOR}>OBS: %obs_scene%</fc> | %date% | %time_norfolk% Norfolk | %time_paris% Paris | %time_la% Los Angeles",
    commands =
       [ Run Date "%a, %d %b %Y" "date" 10
        ...
        ,Run Com "log-hours" ["stats", "dns", "--quiet"] "dns_hours" 10 
